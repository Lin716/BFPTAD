...
class Config(object):
    ...
    """
    def __init__(self, batch=4, is_training=True, clip_overlap=0.75):
        self.epochs = 100
        self.batch = batch
        ...
        self.detection_res = "%s/%s" % (self.res_path, self.dataset)   # 后处理结果文件，保存为txt
        self.ckpt_path = self.ckpt_path + '/' + self.dataset
        self.res_path = self.res_path + '/' + self.dataset
        self.log_path = self.log_path + '/' + self.dataset
