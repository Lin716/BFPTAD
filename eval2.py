...
def calculate_anchor_coordinate_one_layer(feat_map_dim, anchor_scales, anchor_step, anchor_width, offset=0.5):
    """
    计算单个特征图中所有锚点的坐标和尺寸信息
    每层网格生成逻辑:
    生成全部网格中心点坐标，存储下来
    生成一组网格的长宽，存储下来
    最终这一组长宽匹配所有的中心点，生成全部的网格，不过这一步不在网格生成函数中，仅是逻辑步骤
    :param video_length: 输入视频长度
    :param feat_map_dim: 特征图的时间维度长度
    :param anchor_scales: anchor的比率, 不包含默认宽度
    :param anchor_step: 每个特征层相对于clip的缩放比例
    :param anchor_width: 特征图中的每个默认anchor的大小
    :param offset: 锚点中心在每个特征图cell中的偏移
    :return: 中心点坐标和anchor的长度
    """
...
def post_process(video_res, video, f):
    """
    对每个视频的检测结果进行后处理
...
def eval(cfg, data,dataloader,model,ckpt,device):
    """
       eval
       :param cfg: 参数类的对象
       :param ckpt: 模型
    ...
    if not os.path.exists(cfg.ckpt_path+'/ssl'):
        logging.info("The ckpt path is not exists!")
    if not os.path.exists(cfg.detection_res+'/ssl'):
    ...
    # 保存后处理结果文件
    res_file = "%s/res_fps_%s_epoch_%s.txt" % (cfg.detection_res+'/ssl', cfg.fps, epoch)
    f = open(res_file, 'w')
    ...
    for n_iter,(input_data,clip) in enumerate(dataloader):
        ...
        if video_name != clip_video_name:  # 改变视频时保存上一个视频的检测结果
            post_process([one_video_score_res, one_video_loc_res], video_name, f)  # 后处理
        ...
        with torch.no_grad():
            cls, cls_logits, loc, endpoint = model(input_data.to(device))


        # 将预测的部分转成numpy
        for i in range(len(cls)):
            cls[i] = cls[i].cpu().detach().numpy()
            loc[i] = loc[i].cpu().detach().numpy()
...
if __name__ == '__main__':
    ...
    # 文件夹下所有model测试
    for parent, dirnames, filenames in os.walk(cfg.ckpt_path+'/ssl'):
        for i in range(len(filenames)):

            ckpt = cfg.ckpt_path + '/ssl/' + filenames[i]
            print("ckpt:", ckpt)
            eval(cfg, test_data, test_loader, model, ckpt, device)
